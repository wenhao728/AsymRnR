...
def apply_eda(
    ...
    for block in itertools.chain(transformer.transformer_blocks, transformer.single_transformer_blocks):
        if isinstance(block, (HunyuanVideoTransformerBlock, HunyuanVideoSingleTransformerBlock)):
            attn_config = dict(
            ...
            block.attn.set_processor(attention_processor)
...
def apply_rnr(
    ...
    total_num_blocks = len(transformer.transformer_blocks) + len(transformer.single_transformer_blocks)
