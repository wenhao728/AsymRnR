...
def apply_eda(
    ...
    for block in transformer.transformer_blocks:
        if isinstance(block, CogVideoXBlock):
            ...
            block.attn1.set_processor(attention_processor)
...
def _get_timestep_config(
    ...
    for feat in ['h', 'q', 'v', 'k']:
        ...
        for t in range(num_inference_steps):
            ...
            if is_conflict:
                #     continue  # skip conflict features
                h_rnr_ratio = timesteps_config['h'][t]['rnr_ratio']
                if (
                    (feat == 'q' and rnr_ratio_q2h(rnr_ratio) > h_rnr_ratio) or
                    (feat == 'v' and rnr_ratio_v2h(rnr_ratio) > h_rnr_ratio)
                ):  # prune q/v more efficient than h
                    timesteps_config['h'][t] = {'enable': False}  # remove the conflicting feature
                    logger.debug(f"b={block_idx:>2}, t={t:>2}, conflict with {feat}, disabled h")
                else:
                    logger.debug(f"b={block_idx:>2}, t={t:>2}, conflict with {feat}, skip {feat}")
                    continue
